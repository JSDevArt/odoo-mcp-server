<?php

namespace App\Mcp\Tools;

use App\Services\Odoo\OdooClient;
use Illuminate\Contracts\JsonSchema\JsonSchema;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Attributes\Description;
use Laravel\Mcp\Server\Tool;
use Laravel\Mcp\Server\Tools\Annotations\IsIdempotent;
use Laravel\Mcp\Server\Tools\Annotations\IsOpenWorld;
use Throwable;

#[Description('Renames an existing partner / contact (res.partner). Returns previous and new name. WRITES to Odoo.')]
#[IsOpenWorld(true)]
#[IsIdempotent(true)]
class RenamePartnerTool extends Tool
{
    public function handle(Request $request, OdooClient $odoo): Response
    {
        try {
            $partnerId = (int) $request->get('partner_id');
            $newName = (string) $request->get('new_name');

            if ($partnerId <= 0 || $newName === '') {
                return Response::error('partner_id (>0) and new_name (non-empty) are required.');
            }

            $before = $odoo->executeKw('res.partner', 'read', [[$partnerId]], [
                'fields' => ['name', 'email', 'vat'],
            ]);

            if (empty($before)) {
                return Response::error("Partner id={$partnerId} not found.");
            }

            $previousName = $before[0]['name'];

            $ok = $odoo->executeKw('res.partner', 'write', [
                [$partnerId],
                ['name' => $newName],
            ]);

            if (! $ok) {
                return Response::error('write() returned false; no change applied.');
            }

            return Response::json([
                'partner_id' => $partnerId,
                'email' => $before[0]['email'],
                'vat' => $before[0]['vat'],
                'previous_name' => $previousName,
                'new_name' => $newName,
                'status' => 'renamed',
            ]);
        } catch (Throwable $e) {
            return Response::error('rename_partner failed: '.$e->getMessage());
        }
    }

    public function schema(JsonSchema $schema): array
    {
        return [
            'partner_id' => $schema->integer()
                ->description('ID of the partner to rename (from list_partners).')
                ->required(),
            'new_name' => $schema->string()
                ->description('New display name for the partner.')
                ->required(),
        ];
    }
}
